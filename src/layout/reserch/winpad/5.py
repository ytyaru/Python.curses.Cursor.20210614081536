...
class ListWindow:
    def __init__(self, items, x=0, y=0, w=-1, h=-1, attr=0):
        ...
        class ListKeyInput(Input):
            def __init__(self, window):
                ...
                super().__init__()
            def input(self, key):
                if curses.KEY_UP == key: self.__window.ShowY -= 1
                elif curses.KEY_DOWN == key: self.__window.ShowY += 1
                elif curses.KEY_LEFT == key: self.__window.ShowX -= 1
                elif curses.KEY_RIGHT == key: self.__window.ShowX += 1
                else: return False
                return True
        ListKeyInput(self)
    @property
    ...
    def addstrs(self):
        ...
        for i, item in enumerate(self.Items):
            s = f'{self.Items[i + self.ShowY][self.ShowX:]}'
    ...
    def ShowX(self, x):
        ...
        elif l < x: self.__showX = l - self.W + 1
        elif l < x + self.W: self.__showX = l - self.W + 1
        else: self.__showX = x
    @ShowY.setter
    def ShowY(self, y):
    ...
    def init():
        class Window1(Window):
            def draw(self):
                ...
                self.Pointer.addstr(0,0,'Window2',curses.A_REVERSE | curses.color_pair(5))
        win = Window1(x=1,y=1,w=20,h=10)

        l = [f'item-item-item-{i}' for i in range(curses.LINES*3)]
        l.insert(0, '12345678901234567890123456789012345678901234567890123456789012345678901234567890')
        l.insert(0, 'ListWindow')
        lw = ListWindow(l, x=2, y=4, w=20, h=10)
