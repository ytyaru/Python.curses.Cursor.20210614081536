...
class Curses:
    ...
    @classmethod
    def __draw(cls):
        for w in cls.main.WindowManager.Windows: w.Pointer.noutrefresh()
        for p in cls.main.WindowManager.Pads: p.noutrefresh()
        cls.main.draw()
...
if __name__ == "__main__":
    ...
    def draw(wndMgr):
        ...
        wndMgr.Windows[1].Pointer.addstr(9, 0, term.get_parameter(term.get_capability('cup'), 5, 3), curses.color_pair(2+8))
    def loop(wndMgr, key):
    ...
    class Main(CursesMain):
        ...
        def draw(self):
            self.WindowManager.Screen.clear()
            self.WindowManager.Windows[0].Pointer.clear()
            self.WindowManager.Windows[1].Pointer.clear()
            self.WindowManager.Windows[1].Subs[0].Pointer.clear()
            self.WindowManager.Windows[0].Pointer.bkgd(' ', curses.A_REVERSE | curses.color_pair(1))
            self.WindowManager.Windows[1].Pointer.bkgd(' ', curses.A_REVERSE | curses.color_pair(2))
            self.WindowManager.Windows[1].Subs[0].Pointer.bkgd(' ', curses.A_REVERSE | curses.color_pair(3))

            self.WindowManager.Windows[0].Pointer.addstr(0,0,'Window-1', curses.A_REVERSE | curses.color_pair(1))
            self.WindowManager.Windows[0].Pointer.addstr(1, 0, '↑↓←→:move h:hide/show PgUp/PgDn:Z-switch asdw:resize other:quit', curses.color_pair(15))
            self.WindowManager.Windows[1].Pointer.addstr(0,0,'Window-2', curses.A_REVERSE | curses.color_pair(2))
            self.WindowManager.Windows[1].Pointer.addstr(1, 0, '↑↓←→:move h:hide/show PgUp/PgDn:Z-switch asdw:resize other:quit', curses.color_pair(15))
            self.WindowManager.Windows[1].Subs[0].Pointer.addstr(0,0,'Window-2-Sub-1', curses.A_REVERSE | curses.color_pair(3))
        def input(self, key):
