...
class Curses:
    ...
    wait_time = 5
    @classmethod
    def run(cls, pre_init=None, init=None, wait_time=5):
        if pre_init is not None: pre_init()
        cls.init = init
    ...
    def __draw(cls):
        ...
        for p in Pad.Pads: p.noutrefresh(); p.draw();
...
if __name__ == "__main__":
    class Window1(Window):
        def draw(self):
            self.Pointer.clear()
            self.Pointer.bkgd(' ', curses.A_REVERSE | curses.color_pair(1))
            self.Pointer.addstr(0,0,'Window-1', curses.A_REVERSE | curses.color_pair(1))
            self.Pointer.addstr(1, 0, '↑↓←→:move h:hide/show PgUp/PgDn:Z-switch asdw:resize other:quit', curses.color_pair(15))
    class Window2(Window):
        def draw(self):
            self.Pointer.clear()
            self.Subs[0].Pointer.clear()
            self.Pointer.bkgd(' ', curses.A_REVERSE | curses.color_pair(2))
            self.Subs[0].Pointer.bkgd(' ', curses.A_REVERSE | curses.color_pair(3))

            self.Pointer.addstr(0,0,'Window-2', curses.A_REVERSE | curses.color_pair(2))
            self.Pointer.addstr(1, 0, '↑↓←→:move h:hide/show PgUp/PgDn:Z-switch asdw:resize other:quit', curses.color_pair(15))
            self.Subs[0].Pointer.addstr(0,0,'Window-2-Sub-1', curses.A_REVERSE | curses.color_pair(3))
    class SubWindow21(SubWindow):
        def draw(self):
            self.Pointer.addstr(0,0,'Window-2-Sub-1', curses.A_REVERSE | curses.color_pair(3))
    class KeyInput(Input):
        def input(self, key):
            if curses.KEY_UP == key: Window.Windows[1].Y -= 1 if 0 < Window.Windows[1].Y else 0
            elif curses.KEY_DOWN == key: Window.Windows[1].Y += 1 if Window.Windows[1].Y < curses.LINES-Window.Windows[1].H else 0
            elif curses.KEY_LEFT == key: Window.Windows[1].X -= 1 if 0 < Window.Windows[1].X else 0
            elif curses.KEY_RIGHT == key: Window.Windows[1].X += 1 if Window.Windows[1].X < curses.COLS-Window.Windows[1].W else 0
            elif ord('h') == key: Window.Windows[1].switch()
            elif curses.KEY_NPAGE == key or curses.KEY_PPAGE == key:
                if curses.panel.top_panel() == Window.Windows[1].Panel: Window.Windows[0].Panel.top()
                else: Window.Windows[1].Panel.top()
                curses.panel.update_panels()
            elif ord('w') == key: Window.Windows[1].H -= 1 if 1 < Window.Windows[1].H else 0
            elif ord('s') == key: Window.Windows[1].H += 1 if Window.Windows[1].H+Window.Windows[1].Y < curses.LINES else 0
            elif ord('a') == key: Window.Windows[1].W -= 1 if 1 < Window.Windows[1].W else 0
            elif ord('d') == key: Window.Windows[1].W += 1 if Window.Windows[1].W+Window.Windows[1].X < curses.COLS else 0
            else: return False
            return True

    def pre_init(): Terminal.Name = 'xterm-256color'
    def init():
        w1 = Window1(x=2,y=2,w=100,h=10)
        w2 = Window2(x=4,y=4,w=100,h=10)
        w2s1 = SubWindow21(w2, x=2, y=4, w=20, h=4)
        w2.W = 40
        w2.H = 20
        KeyInput()

    Curses.run(pre_init=pre_init, init=init, wait_time=5)
