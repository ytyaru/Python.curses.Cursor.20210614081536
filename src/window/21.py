...
#        cls.main.init()
        windows = cls.main.init()
        for w in windows:
            if isinstance(w, Window) and w not in cls.main.WindowManager.Windows: cls.main.WindowManager.Windows.append(w)
            if isinstance(w, Pad) and w not in cls.main.WindowManager.Pads: cls.main.WindowManager.Pads.append(w)
#            if isinstance(w, Window) and w not in cls.WndMgr.Windows: cls.WndMgr.Windows.append(w)
#            if isinstance(w, Pad) and w not in cls.WndMgr.Pads: cls.WndMgr.Pads.append(w)
        cls.__draw()
        cls.__loop()
    @classmethod
...
#        for w in cls.main.WindowManager.Windows: w.Pointer.noutrefresh(); w.draw();
#        for p in cls.main.WindowManager.Pads: p.noutrefresh(); p.draw();
...
if __name__ == "__main__":
    class Main(CursesMain):
        ...
        def init(self):
            ...
            self.WindowManager.Windows[0].Canvas = Canvas1
            self.WindowManager.Windows[1].Canvas = Canvas2
            """
            w1 = Window1(x=2,y=2,w=100,h=10)
            w2 = Window2(x=4,y=4,w=100,h=10)
            w2s1 = SubWindow21(w2, x=2, y=4, w=20, h=4)
#            w2s1 = SubWindow21(w2.Pointer, x=2, y=4, w=20, h=4)
#            w2.Subs.append(w2s1)
            w2.W = 40
            w2.H = 20
#            self.WindowManager.Windows.append(w1)
#            self.WindowManager.Windows.append(w2)
            return w1, w2
        def draw(self):
#            pass
            for w in Window.Windows: w.draw()
            for p in Pad.Pads: p.draw()
        def input(self, key):
