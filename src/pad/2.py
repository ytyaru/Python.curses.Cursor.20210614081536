...
#        for p in Pad.Pads: p.noutrefresh()
...
#        return self.__parent.Pointer.subpad(h, w, y, x)
#        return self.__parent.Pointer.derwin(h, w, y, x) if is_derwin else self.__parent.Pointer.subwin(h, w, y, x)
    """
...
#        self.__window.refresh(self.__showY, self.__showX, 0, 0, curses.LINES-1, curses.COLS-1)
    def noutrefresh(self):
        self.__window.noutrefresh(self.__showY, self.__showX, 0, 0, curses.LINES-1, curses.COLS-1)
#        for s in self.Subs: s.noutrefresh()
#        for s in self.Subs: s.noutrefresh(s.ShowY, s.ShowX, 0, 0, curses.LINES-1, curses.COLS-1)
    def refresh(self):
        self.__window.refresh(self.__showY, self.__showX, 0, 0, curses.LINES-1, curses.COLS-1)
#        for s in self.Subs: s.refresh()
#        for s in self.Subs: s.refresh(s.ShowY, s.ShowX, 0, 0, curses.LINES-1, curses.COLS-1)
    def init(self): pass
    def draw(self): pass
...
#            for s in self.Subs: s.draw()
            ...
            self.Subs[0].Pointer.bkgd(' ', curses.A_REVERSE | curses.color_pair(5))
            self.Subs[0].Pointer.addstr(
                self.ShowY, 
                self.ShowX, 
                f'Pad-1-Sub-1', 
            ...
            self.Subs[0].Pointer.addstr(
                self.ShowY + 2, 
                self.ShowX + 4, 
                f'{self.ShowX},{self.ShowY} ({self.X},{self.Y}) {self.W},{self.H}', 
                curses.A_REVERSE | curses.color_pair(5))
            """
#            self.refresh()
#            for s in self.Subs: s.draw()
...
#                self.Parent.ShowY + 2, 
#                self.Parent.ShowX + 4, 
                f'{self.Parent.ShowX},{self.Parent.ShowY} {self.ShowX},{self.ShowY} ({self.X},{self.Y}) {self.W},{self.H}', 
                curses.A_REVERSE | curses.color_pair(5))
#            self.refresh()
...
#        pad.make_sub(x=40, y=0, w=curses.COLS-40, h=Pad.Pads[0].H)
